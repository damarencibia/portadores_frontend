export const fetchCargaById = async id => {
    try {
      const { data } = await $axios.get(`/retiros-combustible/${id}`);
  ...
  export const validarRetiro = async (id, { valid, validadoPorId, motivoRechazo = null }) => {
    try {
      ...
      const { data } = await $axios.post(`/retiros-combustible/${id}/validar`, payload);
