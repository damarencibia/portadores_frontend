...
import { fetchUsers, fetchUsersByEmpresa, createUser, updateUser, fetchUserById, deleteUser } from './index' // exportConsumoCombustiblePDF ya no es necesario aquí
...
//data table
const headers = ref([
    { title: "nombre", key: "name", value: "name" },
    { title: "apellidos", key: "lastname", value: "lastname" },
    { title: "email", key: "email", value: "email" },
    { title: "celular", key: "phone", value: "phone" },
    { title: "Rol", key: "roles", value: "roles" },
...
const goToUserDetails = (usuarioId) => {
    router.push(`/usuarios/details/${usuarioId}`);
};
const goToUserEdit = (usuarioId) => {
    router.push(`/usuarios/edit/${usuarioId}`);
};
    ...
    <div>
        <VCard title="Usuarios">
            <VCardText class="d-flex justify-space-between gap-4">
                <VRow class="w-100">
                    <VCol cols="12" sm="6">
                        <VTextField v-model="search" @input="onSearchInput" placeholder="Buscar usuarios"
                    ...
                    <VCol class="d-flex w-100 justify-end" cols="12" sm="6">
                        <VBtn color="primary" :block="isMobile" prepend-icon="ri-add-line"
                            @click="$router.push('/usuarios/add')" class="mr-2">
                            Añadir Usuario
                        </VBtn>
